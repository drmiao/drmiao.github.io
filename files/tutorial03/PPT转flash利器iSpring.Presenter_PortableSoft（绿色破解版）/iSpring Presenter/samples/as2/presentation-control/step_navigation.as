...
this.gotoSlideStepButton.onRelease = function()
{
	if (g_controller)
	{
		var sldIndex:Number = parseInt(gotoSlideIndex.text) - 1;
		var stepIndex:Number = parseInt(gotoStepIndex.text);
		g_controller.gotoSlide(sldIndex);
		g_controller.playFromStep(parseInt(gotoStepIndex.text));
		updatePlayPauseButtons(true);
	}
}
